...
int test10() {
  for (int j = 0; j< 1000;j++)
  {
      ...
      if(resized != NULL){ //indicates that the block was (incorrectly) resized
          return FAILURE;
      }
...
int test08() {
  for (int j = 0; j< 1000;j++)
  {
      ...
      hl_release(heap,block_1); //freeing enough space for hypothetical issues
      block_0 = hl_resize(heap,block_0, (1.5*block_size)); //resizing block
      if(block_0 == NULL){return FAILURE;}//should be able to resize
      print_t("pos3");
      ...
      block_0 = hl_resize(heap,block_0, (0.5*block_size));
      if(block_0 == NULL){return FAILURE;}
      for(int k= 0; k < 0.5*block_size;k++)
      {
          if (block_0[k] != 'p'){return FAILURE;}
      }
      block_2 = hl_resize(heap,block_2,(block_size));//resizing block to same size
      for(int k= 0; k < block_size;k++)
      {
          if (block_2[k] != 't'){return FAILURE;}
      }
      block_2 = hl_resize(heap,block_2,(0.75*block_size));//resizing block to same size
      for(int k= 0; k < 0.75*block_size; k++)
      {
          if (block_2[k] != 't'){return FAILURE;}
      }
  }
...
int test16() {
    ...
    // void* boundary = (void*) &memarea[1024*2];
    char *pointers[NPOINTERS];
    ...
    for (int i = 0; i < n_tries; i++) {
        ...
        if (pointers[index] == NULL)
        {
            ...
            for(int t = 0 ; t < block_size;t++)
            {
              if (array[t] != 's'){return FAILURE;}
            }
            hl_release(heap_start, pointers[index]);
            pointers[index] = NULL;
        }
        int p  = 2*HEAP_SIZE + 1;
        for(p  = 2*HEAP_SIZE + 1; p < (3*HEAP_SIZE - 1);p++)
        {
          // printf("yuea %d\n",p);
          //printf("yes\n");
          if (memarea[p] != 5) {return FAILURE;}
    ...
    for (int i = 0; i < n_tries; i++)
    {
      print_int2(1);
        ...
        if (pointers[index] == NULL)
        {
            ...
            {
              print_int2(3);
              // print_block2((block_header_t*) pointers[index]);
              char* array = (char*) pointers[index];
              for(int j = 0; j < block_size; j++)
              {
                array[j] = 'b';
...
int test18() {
    ...
    for (int i = 0; i < n_tries; i++) {
        int index = random() % NPOINTERS;
        block_size = random() % (HEAP_SIZE/10);
        if (pointers[index] == NULL)
        {
            pointers[index] = hl_alloc(heap_start,  block_size);
            if(pointers[index]!= NULL)
...
//               pointers[index][i] = (char) (pointers[index]);
//               block_sizes[index] = block_size;
//             }
//         }
//         else{
//             if(index % 2 == 0){
//               block_size = random() % NPOINTERS;
//               pointers[index] = hl_resize(heap_start, pointers[index], block_size);
//               block_sizes[index] = block_size;
//             }
//             else{
...
//         for(int j = 0; j < NPOINTERS; j++){
//           for(int b = 0; b < block_sizes[j]; b++){
//             if((int)pointers[j] != (char) pointers[j][b]){
//               return FAILURE;
//             }
//           }
//         }
...
int test20() {
    ...
    for (int i = 0; i < n_tries; i++)
    {
        ...
        if (pointers[index] == 0)
        {
            ...
            for(i = 0; i < block_size; i++)
            {
              pointers[index][i] = characters[index%26];
              block_sizes[index] = block_size;
            }
        }
        else
        ...
        for(int j = 0; j < NPOINTERS; j++)
        {
          for(int b = 0; b < block_sizes[j]; b++)
          {
            if(pointers[j][b] != (characters[j%26]))
            {
              return FAILURE;
            }
          }
